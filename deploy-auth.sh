#!/bin/bash
# deploy.sh for Kitchana-Auth using docker-compose
...
sed -i "s|^AUTH_TAG=.*|AUTH_TAG=$TAG|" .env


# article 컨테이너만 강제 재생성 (기존 컨테이너가 있다면 종료 및 삭제 후 새 컨테이너 실행 / 없을 경우 시작)
docker-compose -f docker-compose-inner.yml up -d --no-deps --force-recreate auth
